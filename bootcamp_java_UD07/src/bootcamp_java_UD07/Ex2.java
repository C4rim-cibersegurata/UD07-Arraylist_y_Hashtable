...
public class Ex2 {
	...
	public static void copiaLista(Hashtable<String, Double> listaCompra,Hashtable<String, Double> productos4iva,Hashtable<String, Double> productos21iva) {
		...
		while (!producto.equals("-")) {
			...
			else if (producto.equals("-")) {System.out.println("saliendo...");}
			else {System.out.println("El producto "+producto+" no esta disponible en la tienda");}
		}
	}
	...
	public static void main(String[] args) {
		...
		if (listaCompra.size()>0) {//si la lista de compra tiene elementos
			...
			if (pago<precioTotalIVA) { //Si no ha pagado lo suficiente se vuelve a pedir el pago
				...
				System.out.println("Debe pagar con una cantidad superior o igual a:"+String.format("%.2f",precioTotalIVA)+" introduce de nuevo el pago");
				pago = sc.nextDouble();
			}
			...
			while (e.hasMoreElements()) {
				...
				if (productos4iva.containsKey(clau)) { //Dependiendo de en que lista se encuentre mostrará  un IVA añadido u otro
					System.out.println(clau+" 4% de IVA aplicado: " + String.format("%.2f",(listaCompra.get(clau) + (listaCompra.get(clau)*0.04))) + "€");
					precioTotal=precioTotal+listaCompra.get(clau);
				}
				else if (productos21iva.containsKey(clau)) {
					System.out.println(clau+" 21% de IVA aplicado: " + String.format("%.2f",(listaCompra.get(clau) + (listaCompra.get(clau)*0.21))) + "€");
					precioTotal=precioTotal+listaCompra.get(clau);
				}
			}
			System.out.println("----------------------------------------------");
